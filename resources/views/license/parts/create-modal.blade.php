<x-modal id="create-license" title="Tambah Lisensi Baru" maxWidth="xl">
    <form action="{{ route('licenses.store') }}" method="POST" x-data="{
        licensePlan: 'starter',
        customType: 'months',
        customValue: 1
    }">
        ...
        <div class="space-y-5">
            {{-- Customer Select --}}
            <div>
                <label for="customer_id" class="block text-sm font-medium text-slate-700 mb-1.5">Customer
                    (Opsional)</label>
            ...
            <x-input label="Domain" name="domain" type="text" placeholder="example.com (kosongkan untuk universal)"
                hint="Biarkan kosong untuk lisensi universal yang bisa digunakan di semua domain" />
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <label class="relative flex cursor-pointer rounded-xl border-2 p-4 transition-all"
                        :class="licensePlan === 'custom' ? 'border-indigo-500 bg-indigo-50 shadow-md shadow-indigo-500/10' :
                            'border-slate-200 hover:bg-slate-50 hover:border-slate-300'">
                        <input type="radio" name="license_plan" value="custom" class="sr-only" x-model="licensePlan">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center"
            ...
            <x-input label="Maksimal Aktivasi" name="max_activation" type="number" value="1" required
                hint="Jumlah perangkat yang bisa mengaktifkan lisensi ini" />
